...
5) Расчет трафика для одного пользовател в день
    - Авторизация: 173 КБ * 1 = 173 КБ, 184 * 1 = 184 запроса
    - Регистрация: 381 КБ * 0.005 = 1.91 КБ, 144 * 0.005 = 0.72 запроса
    - Главная страница: 2.6 МБ * 1 = 2.6 МБ, 72 * 1 = 72 запроса
    - Страница категории: 3.7 МБ * 2 = 7.4 МБ, 381 * 2 = 762 запроса
    - Страница поиского запроса: 3.4 МБ * 2 = 6.8 МБ, 325 * 2 = 650 запроса
    - Страница мои объявления: 907 КБ * 0.1 = 91 КБ, 199 * 0.1 = 19.9 запроса
    - Страница мои заказы: 1.1 МБ * 0.5 = 550 КБ, 241 * 0.5 = 120.5 запроса
    - Страница кошелек: 1.3 МБ * 0.1 = 130 КБ, 294 * 0.1 = 29.4 запроса
    - Страница мои сообщения: 1.4 МБ * 0.3 = 420 КБ, 366 * 0.3 = 109.8 запроса
    - Отправка сообщения: 1 МБ * 1 = 1 МБ, 1 * 1 = 1 запроса
    - Отправка объявления: 5 МБ * 0.25 = 1.25 МБ, 1211 * 0.25 = 302.75 запроса
...
9) Оценка существенных запросов<br>
   - Авторизация: (184 * 832 500) запросов : (23 * 3 600) секунд = 1850 запросов в секунду
   - Регистрация: (0.72 * 832 500) запросов : (23 * 3 600) секунд = 7.24 запросов в секунду
   - Главная страница: (72 * 832 500) запросов : (23 * 3 600) секунд = 723.91 запросов в секунду
   - Страница категории: (762 * 832 500) запросов : (23 * 3 600) секунд = 7661.4 запросов в секунду
   - Страница поиского запроса: (650 * 832 500) запросов : (23 * 3 600) секунд = 6535.32 запросов в секунду
   - Страница мои объявления: (19.9 * 832 500) запросов : (23 * 3 600) секунд = 200.08 запросов в секунду
   - Страница мои заказы: (120.5 * 832 500) запросов : (23 * 3 600) секунд = 1211.54 запросов в секунду
   - Страница кошелек: (29.4 * 832 500) запросов : (23 * 3 600) секунд = 295.59 запросов в секунду
   - Страница мои сообщения: (109.8 * 832 500) запросов : (23 * 3 600) секунд = 1103.96 запросов в секунду
   - Отправка сообщения: (1 * 832 500) запросов : (23 * 3 600) секунд = 10.05 запросов в секунду
   - Отправка объявления: (302.75 * 832 500) запросов : (23 * 3 600) секунд = 3043.95 запросов в секунду
...
#### Хранение данных пользователя:
   ...
   Предлагается для хранения данных пользователя использовать базу данных PostgreSQL. Для авторизацию реализовать с помощью Redis, работающую со структурами данных типа «ключ — значение».

   Условная схема хранения без деталей реализации. Пара «ключ — значение».<br>
     - <b>Ключ:</b> Cookie, email, пароль пользователя.<br>
     - <b>Значение:</b> JSON-подобный объект, создержащий данные сесси. Пример:<br>
   ```yaml
         {
              "id автора": 74654,
...
Наиболее загруженная точка сервиса - это поиск по объявлениям. Будут использованы индексы на название, категории и цену (это самые популярные виды выборок).<br>
...
Расчет количества машин: 30 000 000 * 529 байт = 15 870 000 000 байт = 14.78 Гб<br>
...
<br>
По условия сервиса разрешено хранить изображения до 2 Мб. Среднее количество изображений объявления - 2. При 30 000 000 объявлений получаем 30 000 000 * 2 Мб = 28.61 Тб.

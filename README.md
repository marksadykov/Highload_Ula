...
#### Сервер бекенд
16 ядер + 32 Гб машина с Golang приложением при относительно простых запросах:
```yaml
...
```yaml
1: Перед базой пользователей надо поставить одну машину с 1 ТБ диском (данные сессий)
2: Перед базой объявлений надо поставить 2 машины с 1 ТБ диском (данные сессий)
```

#### Машина пользователей
Для PostgreSQL машины 16 ядер + 32 Гб:
```yaml
...
перед пользователем ставим 2 машины с диском по 500 Гб (данные и репликация), применяется шардирование.

#### Машина объявлений
Для PostgreSQL машины 16 ядер + 32 Гб с максимальными оптимизациями:
```yaml
RPS: 2500 запросов в секунду
подключения: до 10 000
каждая машина содержит группу: 1 машина для записи и 2 машины для чтения
...
перед объявлениями ставим 4 машины с диском по 5 Тб (данные и репликация), применяется шардирование.
